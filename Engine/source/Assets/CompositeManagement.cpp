...
int CKLBCompositeAsset::readInt(long long integerVal) 
{
	...
	case anchor:
		...
		switch (m_pCurrInnerDef->anchor) {
			case 1:
				m_pCurrInnerDef->x = m_pCurrInnerDef->x;
				if (m_width){
			...
			case 2:
				// not sure about this
				if (m_pCurrInnerDef->y < 0) {
					m_pCurrInnerDef->y = m_pCurrInnerDef->y + 1180;
					m_pCurrInnerDef->y += m_pCurrInnerDef->height; 
				}
				else if (m_pCurrInnerDef->y <= 100) {
					m_pCurrInnerDef->y = m_pCurrInnerDef->y - 100;
					m_pCurrInnerDef->y -= m_pCurrInnerDef->height;
				}
				else {
